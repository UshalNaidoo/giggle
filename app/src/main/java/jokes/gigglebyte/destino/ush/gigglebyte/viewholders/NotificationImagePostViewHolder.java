...
public class NotificationImagePostViewHolder extends UserGridViewHolder {
  ...
  public void setData(final Activity activity, BaseAdapter adapter, View convertView,
                      final Post post, OpenScreen screenToOpen) {
    ...
    String infoText = (user.getName() == null || user.getName().isEmpty() ? activity.getResources()
        .getString(R.string.unknown) : user.getName()) + " " + (
                          post.getType() == PostType.LIKE_IMAGE_POST_NOTIFICATION
                          ?
                          activity.getResources().getString(R.string.liked_post)
                          : activity.getResources().getString(R.string.commented_on));
